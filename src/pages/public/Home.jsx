...
import { 
  ...
  ShieldAlert, User, CheckCircle2, ChevronRight, BarChart3, TrendingUp, 
  Sparkles, MapPin, Search, ArrowRight, Lock
} from "lucide-react";
...
export default function Home() {
  ...
  const [loginModalAction, setLoginModalAction] = useState("this feature");

  const handleProtectedAction = (actionLabel) => {
    if (isAuthenticated) return false; // caller should navigate
    setLoginModalAction(actionLabel);
    setLoginModalOpen(true);
    return true; // modal shown, navigation blocked
  };
      ...
      <section className="relative pt-32 pb-20 lg:pt-48 lg:pb-32 flex flex-col items-center text-center px-6 overflow-hidden min-h-[80vh]">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          >
            {isAuthenticated ? (
              <>
                <Link to="/dashboard/upload" className="w-full sm:w-auto px-8 py-3.5 rounded-xl bg-accent text-[#0a0a0f] font-semibold flex items-center justify-center gap-2 hover:bg-accentHover transition-colors shadow-glow-accent group">
                  <Upload className="w-5 h-5 group-hover:-translate-y-1 transition-transform" />
                  Upload Bill
                </Link>
                <Link to="/dashboard/chat" className="w-full sm:w-auto px-8 py-3.5 rounded-xl bg-card border border-border text-white font-semibold flex items-center justify-center gap-2 hover:bg-cardHover transition-colors">
              ...
              <>
                <button 
                  onClick={() => handleProtectedAction("Bill Upload")}
                  className="w-full sm:w-auto px-8 py-3.5 rounded-xl bg-accent text-[#0a0a0f] font-semibold flex items-center justify-center gap-2 hover:bg-accentHover transition-colors shadow-glow-accent group"
                >
                  <Upload className="w-5 h-5 group-hover:-translate-y-1 transition-transform" />
                  Upload Bill
                </button>
                <Link to="/signup" className="w-full sm:w-auto px-8 py-3.5 rounded-xl bg-card border border-border text-white font-semibold flex items-center justify-center gap-2 hover:bg-cardHover transition-colors">
          ...
          {!isAuthenticated && (
            <motion.p 
              ...
              className="flex items-center gap-2 text-xs text-textSecondary mt-4"
            >
              <Lock className="w-3 h-3" />
              Free account required to analyze bills
            </motion.p>
          )}
        </div>
      </section>

      {/* Platform Feature Showcase — MARKETING ONLY */}
      <section id="platform" className="max-w-7xl mx-auto px-6 py-12 space-y-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 }} className="p-8 rounded-3xl bg-card border border-border flex flex-col justify-between">
            ...
            <Link to="/features" className="text-accent font-semibold text-sm flex items-center gap-2 mt-6 group">
            ...
            <div className="flex flex-col sm:flex-row gap-4 mb-6">
              <div className="flex-1">
                ...
                <div className="relative">
                  ...
                  <input type="text" placeholder="e.g., Software Developer" disabled className="w-full bg-[#171a21] border border-border rounded-xl py-3 pl-10 pr-4 text-sm focus:outline-none focus:border-accent opacity-60 cursor-not-allowed" />
                </div>
              </div>
              <div className="flex-1">
                <label className="text-xs text-textSecondary mb-2 block uppercase tracking-wider">Annual Income</label>
                <input type="text" placeholder="e.g., $80,000" disabled className="w-full bg-[#171a21] border border-border rounded-xl py-3 px-4 text-sm focus:outline-none focus:border-accent opacity-60 cursor-not-allowed" />
              </div>
            </div>
            <button 
              onClick={() => handleProtectedAction("Profile Impact Analysis")}
              className="px-6 py-3 rounded-xl bg-accent text-[#0a0a0f] font-semibold text-sm hover:bg-accentHover transition-colors flex items-center gap-2"
            >
              {isAuthenticated 
                ? <Link to="/dashboard" className="flex items-center gap-2">Analyze My Profile</Link>
                : <><Lock className="w-4 h-4" /> Sign in to Analyze</>
              }
            </button>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="p-8 rounded-3xl bg-card border border-border flex flex-col h-full">
            ...
            {/* Demo chat preview — static/read-only on landing */}
            ...
            <div className="relative">
              <button
                onClick={() => {
                  if (!isAuthenticated) handleProtectedAction("AI Chat");
                }}
              >
                <input 
                  type="text" 
                  placeholder={isAuthenticated ? "Type your question..." : "Sign in to ask CivicSync AI..."} 
                  disabled={!isAuthenticated}
                  readOnly={!isAuthenticated}
                  className="w-full bg-[#171a21] border border-border rounded-full py-4 pl-6 pr-14 text-sm focus:outline-none focus:border-accent disabled:opacity-60 disabled:cursor-pointer" 
                />
              </button>
              {isAuthenticated ? (
                <Link to="/dashboard/chat" className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-accent text-[#0a0a0f] flex items-center justify-center hover:bg-accentHover transition-colors">
                  <ChevronRight className="w-5 h-5" />
                </Link>
              ) : (
                <button 
                  onClick={() => handleProtectedAction("AI Chat")}
                  className="absolute right-2 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-accent/50 text-[#0a0a0f] flex items-center justify-center"
                >
        ...
        {/* CTA Section */}
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="py-16 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-accent/30 bg-accent/5 mb-6">
            <Sparkles className="w-3.5 h-3.5 text-accent" />
            <span className="text-xs font-semibold text-accent tracking-widest uppercase">Ready to get started?</span>
          </div>
          <h2 className="text-4xl font-bold mb-4">Join 50,000+ Informed Citizens</h2>
          <p className="text-textSecondary mb-8 max-w-lg mx-auto">Create your free account and start understanding legislation that affects your life — in seconds.</p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Link to="/features" className="px-8 py-4 rounded-xl bg-card border border-border text-white font-semibold hover:bg-cardHover transition-colors">
              See All Features
            </Link>
          </div>
        </motion.div>
