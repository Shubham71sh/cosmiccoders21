import {
  ...
  CheckCircle2, ChevronRight, Activity, Calendar,
...
import { getDashboardSchemesCount } from "../../firebase/dashboardFirestore";
...
export default function DashboardOverview() {
  ...
  // ── Derived values ─────────────────────────────────────────────────────────
  const billsProgress = billsCount
    ? Math.min(100, Math.round((billsCount / 1500) * 100))
    : 0;

  const schemesProgress = schemesCount
    ? Math.min(100, Math.round((schemesCount / 60) * 100))
    : 0;

  const topScheme1 = eligibility?.topSchemes?.[0] ?? null;
  const topScheme2 = eligibility?.topSchemes?.[1] ?? null;

  // Impact chart: use real analytics bars or zeros
  const impactBars = analytics?.monthlyApplications
    ? analytics.monthlyApplications.map((m) => {
        ...
        return Math.round(((m.applications || 0) / maxVal) * 90) || 5;
      })
    : [0, 0, 0, 0, 0, 0];
    ...
    <div className="space-y-6 pb-20">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 border-b border-border pb-6">
        <div className="flex items-center gap-4">
          <h1 className="text-2xl font-bold text-white tracking-tight">Citizen Overview</h1>
        ...
        <div className="flex items-center gap-4">
          {/* Search Bar — live against bills + schemes */}
          ...
          <div className="absolute bottom-0 left-0 right-0 h-1 bg-border">
            <div
              ...
              style={{
                width: loading
                  ? "0%"
                  : unclaimedBenefits?.count > 0
                    ? `${Math.min(unclaimedBenefits.count * 10, 100)}%`
                    : "5%",
              }}
            />
          </div>
                  ...
                  <>
                    ...
                    {latestBill.analyzedAt || latestBill.createdAt ? (
                      <p className="text-[10px] text-textMuted mb-3">
                        Analyzed:{" "}
                        {new Date(latestBill.analyzedAt || latestBill.createdAt).toLocaleDateString()}
                      </p>
                    ) : null}
                    <div className="flex gap-3">
                      <button
          ...
          >
            ...
            <div className="relative z-10">
              ...
              {loading ? (
                ...
                <p className="text-xs text-textSecondary mt-1 leading-tight">
                  {gps.currentStage}
                </p>
              ) : (
